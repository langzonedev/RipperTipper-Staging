(() => {
  ...
  function render(snapshot) {
    ...
    el.dataStatus.textContent = `${ts ? freshness(ts) : safe(snapshot.updated_label, "Staging update")} · ${safe(snapshot.status, "Current picks")}`;
  ...
  async function load() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`The staging picks returned an error (${response.status}).`);
